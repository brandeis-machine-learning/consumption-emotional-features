...
def evaluate_all(df_feature, feat, pred_var):
    ...
    for test in range(STARTING_TEST, len(x)):
        ...
        y_pred.append(torch.abs(pred[-1][-1]-split_y[test-step]).cpu().detach().numpy())
...
def evaluate_lstm(df_feature, feat, pred_var):
    ...
    for test in range(STARTING_TEST, len(x)):
        ...
        # model.add(Dropout(0.1))
        model.add(Dense(1))
        model.compile(loss='mean_squared_error', optimizer=optimizers.Adam(lr=0.00003))
...
features = [['pre_viewcount'],
            ['pre_viewcount','dummy','baidu_index','survey','is_first'],
            ['dummy','baidu_index','survey','is_first','pre_viewcount','pre_textcomment','pre_emojicomment','pre_all_negative_duration','pre_pos_k',
