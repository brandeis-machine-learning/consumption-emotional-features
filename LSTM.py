...
BASELINE = ['pre_viewcount', 'pre_textcomment', 'pre_emotioncomment']
...
def evaluate_LSTM(var):
    ...
    for pred_var in PREDICT_VAR:
        ...
        for test in range(STARTING_TEST, len(x)):
            ...
            model.fit(split_x[:test-step], split_y[:test-step], epochs=1000, verbose=0, shuffle=False, callbacks=[es])
            pred = model.predict(split_x[test-step:test-step+1])
            y_pred.append(np.abs(pred[0][0]-split_y[test-step]))
            print(pred, split_y[test-step], y_pred[-1])
        rmse = np.sqrt(np.mean(np.square(np.array(y_pred))))
        res.append(np.array(y_pred))
        res_rmse.append(rmse)
    return np.array(res), np.array(res_rmse)
...
def evaluate_BASELINE(var):
    step = 10
    feature_pd = pd.read_excel("feature_" + str(SELECT_PERCENT) + ".xlsx",encoding='utf-8',index=False)
    res, res_rmse = [], []
    for i in range(len(PREDICT_VAR)):
        pred_var = PREDICT_VAR[i]
        x = feature_pd[[var[i]]].values
        y = feature_pd[pred_var].values
...
def evaluate(var):
    ...
    for _ in range(1):
        ...
        for test in range(62, 68):
            ...
            model.fit(x[:test], y[:test], epochs=1000, verbose=0, shuffle=False, callbacks=[es])
            pred = model.predict(x[test:test+1])
            y_pred.append(np.abs(pred[0][0]-y[test]))
            print(pred, y[test], y_pred[-1])
        rmse = np.sqrt(np.mean(np.square(np.array(y_pred))))
        res.append(np.array(y_pred))
        res_rmse.append(rmse)
    return np.array(res), np.array(res_rmse)
...
def evaluate_all(var):
    # generate_tables()
    res, rmses = evaluate_BASELINE(var)
    print(rmses)
    return res, rmses
    # np.savetxt('rmse_pos.csv', np.abs(res.T), delimiter=',')
...
for seed in range(1):
    ...
    res_cur, rmse_cur = evaluate_all(BASELINE)
    all_res.append(res_cur)
    sig_rmse.append(rmse_cur)

all_res = np.array(all_res)
sig_rmse = np.array(sig_rmse)
for i in range(3):
    np.savetxt('result_6_only_rmse_' + PREDICT_VAR[i] + '.csv', all_res[:,i,:], delimiter=',')
print('sig_rmse:', np.mean(sig_rmse, axis=0))
