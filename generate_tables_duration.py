...
def get_peak(y,episode=24,lag=20,threshold=1,influence=0.5):
    lag = 15
    result = thresholding_algo(y, lag=lag, threshold=WINDOW_SIZE, influence=influence)
...
def generate_tables():
    ...
    if NUM_SEPT >= 3:
        ...
        new_save['Season'] = feature_pd['Season']
        new_save['Episode'] = feature_pd['Episode']
        new_save['viewcount'] = feature_pd['viewcount']
        new_save['dummy'] = feature_pd['dummy']
        new_save['pre_viewcount'] = feature_pd['pre_viewcount']
        new_save['pre_textcomment'] = feature_pd['pre_textcomment']
        new_save['pre_emojicomment'] = feature_pd['pre_emojicomment']
        new_save['pre_emotioncomment'] = feature_pd['pre_emotioncomment']
        new_save['pre_all_positive_duration'] = feature_pd['pre_all_positive_duration']
        new_save['pre_all_negative_duration'] = feature_pd['pre_all_negative_duration']
        new_save['pre_pos_k'] = feature_pd['pre_pos_k']
        new_save['pre_neg_k'] = feature_pd['pre_neg_k']

        for i in range(1, NUM_SEPT-1):
            new_save[str(i) + '_all_positive_duration'] = feature_pd[str(i) + '_all_positive_duration']
            new_save[str(i) + '_all_negative_duration'] = feature_pd[str(i) + '_all_negative_duration']
            new_save[str(i) + '_pos_k'] = feature_pd[str(i) + '_pos_k']
            new_save[str(i) + '_neg_k'] = feature_pd[str(i) + '_neg_k']
            new_save[str(i) + '_pos_neg'] = feature_pd[str(i) + '_pos_neg']
            new_save[str(i) + '_minus'] = feature_pd[str(i) + '_pos_max'] - feature_pd[str(i) + '_neg_max']
            vec = new_save[str(i) + '_minus']
            new_vec = []
            for v in vec:
                if v < 0:
                    new_vec.append(np.log10(-v))
                elif v == 0:
                    new_vec.append(0)
                else:
                    new_vec.append(np.log10(v))
            new_save[str(i) + '_minus'] = new_vec
        
        new_save.to_csv("feature_" + str(SELECT_PERCENT) + "_" + str(NUM_SEPT) + "_sigma_" + str(WINDOW_SIZE) + ".csv", index=None)
