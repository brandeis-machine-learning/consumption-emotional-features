...
def my_interpolation(time_series_1, time_series_2):
    x1 = np.linspace(0, 1, len(time_series_1))
    x2 = np.linspace(0, 1, len(time_series_2))

    interpolator = interp1d(x2, time_series_2, kind='linear', fill_value="extrapolate")
    time_series_2_resampled = interpolator(x1)
    return time_series_2_resampled
    ...
    for season in range(1, 7):
        for total_episode in range(1,NUM_EPISODE[season-1]):
            ...
            if season==1 and total_episode==1:
                ...
                # dtw_pos, _ = fastdtw(emotion_positive, pre_emotion_positive, dist=euclidean)
                # dtw_neg, _ = fastdtw(emotion_negative, pre_emotion_negative, dist=euclidean)

                pre_emotion_positive_new = my_interpolation(emotion_positive, pre_emotion_positive)
                pre_emotion_negative_new = my_interpolation(emotion_negative, pre_emotion_negative)

                dtw_pos = np.corrcoef(emotion_positive, pre_emotion_positive_new)[0, 1]
                dtw_neg = np.corrcoef(emotion_negative, pre_emotion_negative_new)[0, 1]
                feature['DTW_pos'].append(dtw_pos)
                feature['DTW_neg'].append(dtw_neg)
            ...
            pre_emotion_negative = emotion_negative
            pre_emotion_positive = emotion_positive

    # for key in feature.keys():
    #     print(key, len(feature[key]))
    feature_pd = pd.DataFrame.from_dict(feature)
    feature_pd.to_excel("feature_" + str(SELECT_PERCENT) + "_" + str(NUM_SEPT) + "_DTW.xlsx",encoding='utf-8',index=False, engine='xlsxwriter')
